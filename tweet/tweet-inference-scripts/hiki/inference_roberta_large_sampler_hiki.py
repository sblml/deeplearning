...
model1.to(device)
model1.load_state_dict(torch.load(f"../input/tweet-roberta-large-v196/ckpt_v1.96_0.bin"))
model1.eval()

model2 = TweetModel(conf=model_config)
model2.to(device)
model2.load_state_dict(torch.load(f"../input/tweet-roberta-large-v196/ckpt_v1.96_1.bin"))
model2.eval()

model3 = TweetModel(conf=model_config)
model3.to(device)
model3.load_state_dict(torch.load(f"../input/tweet-roberta-large-v196/ckpt_v1.96_2.bin"))
model3.eval()

model4 = TweetModel(conf=model_config)
model4.to(device)
model4.load_state_dict(torch.load(f"../input/tweet-roberta-large-v196/ckpt_v1.96_3.bin"))
model4.eval()

model5 = TweetModel(conf=model_config)
model5.to(device)
model5.load_state_dict(torch.load(f"../input/tweet-roberta-large-v196/ckpt_v1.96_4.bin"))
model5.eval()
...
with open(f"hiki_roberta-large_sampler-char_pred_test_start.pkl", "wb") as fp:   #Pickling
    pickle.dump(char_pref_pred_start, fp)
    
with open(f"hiki_roberta-large_sampler-char_pred_test_end.pkl", "wb") as fp:   #Pickling
    pickle.dump(char_pref_pred_end, fp)