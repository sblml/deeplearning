...
class ConfigBertBase:
    ...
    weights = sorted([f for f in os.listdir(weights_path) if "albert" not in f and "distil" not in f])
#     print(weights)
    name = 'bert-base-neutral'
...
def main():
    ...
    for config in configs:
        ...
        with open('bert-base-uncased-char_pred_test_start.pkl', 'wb') as handle:
            pickle.dump(char_pred_test_start, handle)
        with open('bert-base-uncased-char_pred_test_end.pkl', 'wb') as handle:
