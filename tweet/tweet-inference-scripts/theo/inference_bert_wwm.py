...
class ConfigBertWWM:
    # Architecture
    selected_model = "bert-large-uncased-whole-word-masking-finetuned-squad"
    lowercase = True
    nb_layers = 8
    nb_ft = 128
    sentiment_ft = 0
    ...
    weights_path = '../input/tweet-checkpoints-3/'
    weights = sorted(os.listdir(weights_path))
    name = 'bert-wwm-neutral'
...
def main():
    ...
    for config in configs:
        ...
        with open('bert-wwm-neutral-char_pred_test_start.pkl', 'wb') as handle:
            pickle.dump(char_pred_test_start, handle)
        with open('bert-wwm-neutral-char_pred_test_end.pkl', 'wb') as handle:
