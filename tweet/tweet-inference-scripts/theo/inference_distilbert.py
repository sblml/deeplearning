...
class ConfigDistil:
    # Architecture
    selected_model = "distilbert" #-base-uncased-distilled-squad
    lowercase = True
    nb_layers = 6
    nb_ft = 128
    sentiment_ft = 0
    ...
    weights = sorted([f for f in os.listdir(weights_path) if "distil" in f])
...
def main():
    ...
    for config in configs:
        ...
        with open('distilbert-base-uncased-distilled-squad-char_pred_test_start.pkl', 'wb') as handle:
            pickle.dump(char_pred_test_start, handle)
        with open('distilbert-base-uncased-distilled-squad-char_pred_test_end.pkl', 'wb') as handle:
